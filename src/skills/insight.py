"""Unified insight orchestration knowledge skill."""

from __future__ import annotations

from typing import Any

from ..research.compose import compose_insight
from .schema import skill_result

SKILL_ID = "insight"
TITLE = "决策解读"
PRIORITY = 50


def analyze(
    *,
    meta: dict[str, Any] | None,
    analyzers: list[dict[str, Any]],
    state: str,
    level: str,
    direction: str,
    score: int,
) -> dict[str, Any]:
    payload = compose_insight(
        meta=meta,
        analyzers=analyzers,
        state=state,
        level=level,
        direction=direction,
        score=score,
    )
    if payload.get("status") == "unavailable":
        return skill_result(
            SKILL_ID,
            TITLE,
            payload.get("summary") or "暂无解读。",
            status="unavailable",
            priority=PRIORITY,
        )

    result = skill_result(
        SKILL_ID,
        TITLE,
        payload["summary"],
        priority=score or PRIORITY,
        signals=payload.get("signals") or [],
    )
    result["brief"] = payload.get("brief") or {}
    result["core_hits"] = payload.get("core_hits") or []
    result["gaps"] = payload.get("gaps") or []
    result["judgment_note"] = payload.get("judgment_note") or ""
    result["selected_steps"] = payload.get("selected_steps", 0)
    result["total_steps"] = payload.get("total_steps", 0)
    result["personas"] = payload.get("personas") or []
    return result
